...
class ElasticsearchService:
    ...
    def index_document(
        self, bk_obj_id: str, doc_id: int, document: Dict[str, Any]
    ) -> Dict:
        """Index a document"""
    ...
    def search_documents(
        self, bk_obj_id: str, query: Dict[str, Any], page: int = 1, page_size: int = 20
    ) -> Dict:
        ...
        try:
            result = self.client.search(
                index=index_name, body=query, from_=from_index, size=page_size
            )
            return result
        except Exception:
