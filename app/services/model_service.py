...
from app.services.query_builder import ESQueryBuilder, IndexMappingBuilder
...
class ModelService:
    ...
    def create_tree_object(self, request: TreeObjectCreateRequest) -> Dict:
        ...
        es_service.client.index(
            index=self.model_index, id=request.bk_obj_id, document=doc
        )
    ...
    def list_tree_objects(self, page: int = 1, page_size: int = 20) -> Dict:
        ...
        try:
            ...
            result = es_service.client.search(
                index=self.model_index, body=query, from_=from_index, size=page_size
            )
