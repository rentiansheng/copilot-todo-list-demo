...
class ESQueryBuilder:
    ...
    @staticmethod
    def bool_should(conditions: List[Dict[str, Any]], minimum_should_match: int = 1) -> Dict[str, Any]:
...
class TreeQueryBuilder:
    ...
    @staticmethod
    def find_children_by_parent(parent_obj_id: str, parent_inst_id: int) -> Dict[str, Any]:
        ...
        nested_query = ESQueryBuilder.nested(
            "cw__relation",
            ESQueryBuilder.bool_must(nested_conditions)
        )
        
        return ESQueryBuilder.build_query(
            ESQueryBuilder.bool_must([nested_query])
        )
    
    @staticmethod
    def add_filters_to_query(query: Dict[str, Any], filters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_latest_id() -> Dict[str, Any]:
        ...
        return {
            "size": 1,
            "sort": ESQueryBuilder.sort_by("bk_inst_id", "desc")
        }
    ...
    def find_by_status(status: int) -> Dict[str, Any]:
        ...
        return ESQueryBuilder.build_query(
            ESQueryBuilder.term("cw_status", status)
        )
